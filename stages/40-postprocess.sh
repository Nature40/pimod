...
SHRINK() {
  ...
  tune2fs_output=$(tune2fs -l "${loop_device}")
  local blocksize
  ...
  resize2fs -p "${loop_device}" "${target_blocks}" || (
    ERR="${?}"
    echo -e "\033[0;31m### Error: resize2fs failed (${ERR}).\033[0m"
    umount_image "${loop}"
    return 12
  )
  ...
  # Get partition information and calculate new partition size in sectors
  local partinfo
  partinfo=$(sfdisk -l "${DEST_IMG}" -o START,SECTORS,TYPE 2>/dev/null | grep -E '^ *[0-9]' | sed -n "${IMG_ROOT}p")
  local partstart
  partstart=$(echo "${partinfo}" | awk '{print $1}')
  local parttype
  parttype=$(echo "${partinfo}" | awk '{print $3}')
  ...
  sfdisk -d "${DEST_IMG}" | awk -v part="${IMG_ROOT}" -v newsize="${newsize_sectors}" '
    /^[^#]/ && /start= *[0-9]+, *size= *[0-9]+/ {
      partcount++
      if (partcount == part) gsub(/size= *[0-9]+/, "size=" newsize)
    }
    { print }
  ' | sfdisk --no-reread --force "${DEST_IMG}" 2>&1 || (
    ERR="${?}"
    echo -e "\033[0;31m### Error: sfdisk failed (${ERR}).\033[0m"
    return 13
  )
